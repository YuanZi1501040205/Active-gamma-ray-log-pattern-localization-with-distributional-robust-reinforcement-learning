...
class LazyPrioritizedMultiStepMemory(LazyMultiStepMemory):
    ...
    # drsac code: count the state visiting times
    # based on the https://github.com/bandofstraycats/dr-sac
        self._n_s_dict = dict()
        self._bins = np.linspace(0.0, 5.0, num=10, endpoint=True)

    def _state_to_str(self, state):
        """Create state representation as a string by joining bucket identifiers."""
        return ' '.join(map(str, state))

    def _bucketize(self, state):
        """Bucketize state dimensions."""
        return np.digitize(state, self._bins, right=True)

    def _update_state_count(self, state, update_qty):
        """Increase the state string visit counter."""
        state_str = self._state_to_str(self._bucketize(state))
        if state_str not in self._n_s_dict:
            self._n_s_dict[state_str] = 0
        self._n_s_dict[state_str] += update_qty

    def _get_state_count(self, states):
        """Get the state visit counter."""
        state_count = []
        for i in range(states.shape[0]):
            state_str = self._state_to_str(self._bucketize(states[i]))
            state_count.append(self._n_s_dict[state_str])
        return np.array(state_count) # return a list of state visit times
    ...
    def _append(self, state, action, reward, next_state, done, pa):
        ...
        self._update_state_count(state, 1)#drsac: count the states' visit times
