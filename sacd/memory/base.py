...
class LazyMemory(dict):
    ...
    def _sample(self, indices, batch_size):
        ...
        states_cache = np.copy(states) # output numpy array for state visiting count
        states = torch.Tensor(states).to(self.device).float() / 5.
        next_states = torch.Tensor(
            next_states).to(self.device).float() / 5.
        ...
        return states, actions, rewards, next_states, dones, states_cache
