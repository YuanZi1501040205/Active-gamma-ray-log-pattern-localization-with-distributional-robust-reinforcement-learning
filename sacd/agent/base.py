...
class BaseAgent(ABC):
    ...
    def __init__(self, env, test_env, log_dir, num_steps=100000, batch_size=64,
        ...
        self.device = torch.device(
            "cuda:1" if cuda and torch.cuda.is_available() else "cpu")
    ...
    def train_episode(self):
        ...
        while (not done) and episode_steps <= self.max_episode_steps:
            ...
            if self.start_steps > self.steps:
                ...
                action = self.explore(state) # input np array inside normalization

            next_state, reward, done, _ = self.env.step(action)


            # To calculate efficiently, set priority=max_priority here.
            self.memory.append(state, action, reward, next_state, done) # store truce look
    ...
    def evaluate(self):
        ...
        self.writer.add_scalar(
            'accuracy/test', accuracy, self.steps)
        self.writer.add_scalar(
            'ave_steps/test', average_episode_length, self.steps)
        print('-' * 60)
