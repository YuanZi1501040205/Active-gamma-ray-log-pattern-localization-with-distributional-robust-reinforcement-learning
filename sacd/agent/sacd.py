...
class SacdAgent(BaseAgent):
    ...
    def explore(self, state):
        # Act with randomness.
        state = torch.Tensor(
            state[None, ...]).to(self.device).float() / 5.
        with torch.no_grad():
    ...
    def calc_target_q(self, states, actions, rewards, next_states, dones, states_cache):
        with torch.no_grad():
            actions, action_probs, log_action_probs = self.policy.sample(next_states)
            next_q1, next_q2 = self.target_critic(next_states)

            # drsac code: Distributional robust reward shaping
            # based on the https://github.com/bandofstraycats/dr-sac
            C = 1
            eta = 0.5
            curr_q1, curr_q2 = self.calc_current_q(states, actions, rewards, next_states, dones, states_cache)
            states_array = states_cache
            N_s = self.memory._get_state_count(states_array)
            adv_eps = C / N_s ** eta
            min_q_current = torch.min(curr_q1, curr_q2)
            min_q_next = torch.min(next_q1, next_q2)
            q_mean_state = torch.sum(min_q_current * actions, dim=1)
            q_mean_state_ = torch.sum(min_q_next * actions, dim=1)
            q_mean_state = q_mean_state.unsqueeze(1)
            q_mean_state_ = q_mean_state_.unsqueeze(1)
            q_std = torch.sqrt(torch.sum((min_q_current - q_mean_state) ** 2, dim=1))  # variance of state's q
            q_std_targ = torch.sqrt(torch.sum((min_q_next - q_mean_state_) ** 2, dim=1))  # variance of next state's q
            adv_reward_cor = torch.tensor(np.sqrt(adv_eps / 2.0)).to(q_std.device) * (self.gamma_n * q_std_targ - q_std)
            next_q = (action_probs * (
                torch.min(next_q1, next_q2) - self.alpha * log_action_probs
                )).sum(dim=1, keepdim=True)

        assert rewards.shape == next_q.shape
        return rewards + (1.0 - dones) * self.gamma_n * next_q + adv_reward_cor # adv_reward_cor is for drsac
